...
class UserCountChart extends Component{
    static propTypes = {
    ...
    constructor(props) { 
        ...
        this.GetUserCountStatistics = () => {
            ...
            axios.post(Backend_Server_Address + 'getUserCountStatistics', data, { headers: { 'access_token': cookies.get(Access_Token_Cookie_Name) }  })
            .then((res) => {
                let result = res.data
                // set user count statistics to state
                this.setState({user_count_statistics: result})
                this.LoadingOff()
            }).catch((error) => {
                console.log(error)
                if (error.response){ // server responded with a non-2xx status code
                    ...
                    if(
                        ...
                        this.NetworkErrorScreenOn(notification_message, this.GetUserCountStatistics)
                    }
                }else if (error.request){ // request was made but no response was received ... network error
                    Notification(Network_Error_Message, 'error')
                    this.NetworkErrorScreenOn(Network_Error_Message, this.GetUserCountStatistics)
                }else{ // error occured during request setup ... no network access
                    Notification(No_Network_Access_Message, 'error')
                    this.NetworkErrorScreenOn(No_Network_Access_Message, this.GetUserCountStatistics)
                }
                this.LoadingOff()
            })
        }
    ...
    render() {
        return (
            <div>
                <Helmet>
                    <title>User Count Chart | {Platform_Name}</title>
                ...
                <ToastContainer />
                {
                    ...
                    : <div>
                        ...
                        <Row style={{margin: '0px'}}>
                            ...
                            <Col sm='3'>
                                <br/>
                                <Button onClick={this.GetUserCountStatistics} 
                        ...
                        <div style={{width: '100%', overflowX: 'scroll'}}>
                            <LineChart
                                width={1000}
                                height={300}
                                data={this.state.user_count_statistics}
                                margin={{
...
export default withCookies(UserCountChart);