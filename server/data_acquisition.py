...
def acquire_data(symbol, timeframes, call_module): # call module = training / prediction
    ...
    for timeframe in timeframes:
        ...
        if data_source == 'csv': 
            ...
            ohlc_data_dict[timeframe] = yahoo_fetch_data(symbol, timeframe, timezone_from, timezone_to).iloc[:-1] # removing the last row since its a bar still forming, as stated above
            if call_module == 'prediction': ohlc_data_dict[timeframe] = ohlc_data_dict[timeframe].tail(number_of_bars_needed)
        elif data_source == 'mt5': 
            from mt5_data import mt5_fetch_data
            ohlc_data_dict[timeframe] = mt5_fetch_data(symbol, timeframe, timezone_from, timezone_to, symbol_type).iloc[:-1] # removing the last row since its a bar still forming, as stated above
            if call_module == 'prediction': ohlc_data_dict[timeframe] = ohlc_data_dict[timeframe].tail(number_of_bars_needed)
